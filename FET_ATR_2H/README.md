# AVAX ATR Live Trading – ATR + Super Trend Strategy

AVAX ATR Live Trading Signal Generator using ATR + Super Trend strategy for AVAXUSDT futures trading with configurable take-profit and stop-loss.

## 🎯 Approach

- **Target**: AVAXUSDT futures trading with ATR + Super Trend signals
...
Download 2-hour klines from Binance:

```bash
python scripts/download_binance_klines.py --symbol AVAXUSDT --interval 2h --start 2024-01-01
```

Data saved to `data/AVAXUSDT_2h.csv` with columns: `time,open,high,low,close,volume`.
...
make train
# or
python scripts/train_runner.py --symbol AVAXUSDT --timeframe 2h
```
...
Configuration: `configs/train_2h.json`
...
make grid
# or
python scripts/gridsearch_runner.py --symbol AVAXUSDT --timeframe 2h
```
...
make backtest
# or
python scripts/backtest_runner.py --symbol AVAXUSDT --timeframe 2h \
...
make live
# or
python scripts/live_demo_runner.py --symbol AVAXUSDT --timeframe 2h \
...
The production process runs via `run_llm_live.py` and a systemd service. It loads the trained model, fetches new 2h bars, makes decisions, sends Futures orders with TP/SL, and posts Telegram alerts.

- Entry point: `run_llm_live.py`
- Service: `avax_atr_live.service` (systemd)
- Working directory: `/root/ATR/AVAX_ATR_2H`
- Logs: `runs/avax_atr_live.log`
...
- Reads training/live settings from `configs/train_2h.json` (e.g. `timeframe`, `window`, `tp_pct`).
...
```bash
# Status
systemctl status avax_atr_live.service --no-pager -l

# Start/Stop/Restart
systemctl start avax_atr_live.service
systemctl stop avax_atr_live.service
systemctl restart avax_atr_live.service

# Enable on boot
systemctl enable avax_atr_live.service
```

### Logs

```bash
tail -f runs/avax_atr_live.log
```

Typical startup lines:

```
INFO:__main__:🚀 Starting AVAX ATR Live Signal Generator
INFO:src.live_loop:✅ Order client initialized
INFO:__main__:Model loaded: 17 features
INFO:__main__:📊 Monitoring: AVAXUSDT 2h
```
...
- Bar cadence: runs when a new bar appears for the configured `timeframe` (default 2h). Sleeps between checks.
...
- State: Idempotent order client persists to `runs/avax_atr_state.json` and dedupes/retries.

## 🧰 Troubleshooting

- No logs: check service status and permissions in `/root/ATR/AVAX_ATR_2H`.
...
- **Funding rates**: Not modeled in backtest (AVAXUSDT perp funding ~0.01% per 8h)
