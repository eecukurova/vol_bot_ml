#!/usr/bin/env python3
"""
Pengu EMA Trading Bot - Symbol/Market Mapping Helper
...
class SymbolMapping:
    ...
    rest_symbol: str          # REST API için (örn: PENGUSDT)
    order_symbol: str         # Order API için (örn: PENGU/USDT:USDT)
    market_type: str          # market type (swap, spot, etc.)
...
class SymbolMappingHelper:
    """Sembol haritalama ve doğrulama yardımcısı"""
    ...
    def load_and_validate_markets(self, symbol: str) -> SymbolMapping:
        ...
        Args:
            symbol: Config'ten gelen sembol (örn: PENGU/USDT)
    ...
    def _validate_symbol(self, symbol: str) -> SymbolMapping:
        ...
        amount_step = market.get('amount', {}).get('step', 0)
        price_tick = market.get('price', {}).get('step', 0)
    ...
    def _create_symbol_mapping(self, symbol: str, market: Dict) -> SymbolMapping:
        ...
        """
        # REST API sembolü (genelde PENGUSDT)
        rest_symbol = market.get('id', symbol.replace('/', ''))
        
        # Order API sembolü (genelde PENGU/USDT:USDT)
        order_symbol = symbol
        if ':' not in order_symbol:
            # Binance futures için genelde :USDT eklenir
            order_symbol = f"{symbol}:USDT"
    ...
    def get_symbol_for_endpoint(self, endpoint: str) -> str:
        ...
        if endpoint_lower in ['fetch_positions', 'fetch_balance', 'fetch_account']:
            ...
            return self.symbol_mapping.rest_symbol
                               ...
                               'fetch_my_trades', 'fetch_order', 'fetch_orders']:
            # Order API için
...
def test_symbol_mapping():
    ...
    # Test config
    test_configs = [
        {"symbol": "PENGU/USDT", "expected": "valid"},
        {"symbol": "NOPE/USDT", "expected": "not_found"},
        {"symbol": "BTC/USDT", "expected": "valid"},
        {"symbol": "ETH/USDT", "expected": "valid"},
    ]
    
    # Exchange setup
    exchange = ccxt.binance({
        'sandbox': True,  # Test için sandbox
        'apiKey': 'test',
        'secret': 'test'
    })
