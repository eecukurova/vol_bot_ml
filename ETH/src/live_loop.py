    ...
    try:
        ...
        if not positions_found:
            try:
                ...
                # Filter for ETH positions
                for pos in all_positions:
                    pos_symbol = pos.get('symbol', '')
                    if 'ETH' in pos_symbol and 'USDT' in pos_symbol:
...
def send_order(
    ...
    try:
        ...
        # Place entry order
        logger.info(f"Placing {order_side} order for {amount} ETH")
...
def send_telegram_alert(payload: Dict) -> None:
    ...
    try:
        ...
        position_info = ""
        if payload.get("position_opened") is False:
            active_pos = payload.get("active_position")
            if active_pos:
                position_info = f"\n⏸️ Pozisyon açılmadı: Aktif {active_pos} pozisyon mevcut\n"
        ...
        message = f"""
🤖 LLM Futures Signal{shadow_indicator}{position_info}
📌 Pair: {symbol}
📊 Side: {side}
💰 Entry: ${entry:,.2f}
🎯 TP: ${tp:,.2f}
