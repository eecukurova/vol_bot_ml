...
A Transformer-based sequence classifier for ETHUSDT futures scalping with 0.5% take-profit targets and adaptive stop-loss via grid search.

## 🎯 Approach

- **Target**: 0.5% TP scalping on ETHUSDT
...
```bash
python scripts/download_binance_klines.py --symbol ETHUSDT --interval 3m --start 2024-01-01
```

Data saved to `data/ETHUSDT_3m.csv` with columns: `time,open,high,low,close,volume`.
...
make train
# or
python scripts/train_runner.py --symbol ETHUSDT --timeframe 3m
```
...
make grid
# or
python scripts/gridsearch_runner.py --symbol ETHUSDT --timeframe 3m
```
...
make backtest
# or
python scripts/backtest_runner.py --symbol ETHUSDT --timeframe 3m \
...
make live
# or
python scripts/live_demo_runner.py --symbol ETHUSDT --timeframe 3m \
...
INFO:__main__:📊 Monitoring: ETH/USDT 3m
```
...
- **Funding rates**: Not modeled in backtest (ETHUSDT perp funding ~0.01% per 8h)
