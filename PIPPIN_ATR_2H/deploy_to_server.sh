#!/bin/bash
# PIPPIN ATR Projesi - Sunucuya Deploy Scripti

set -e

echo "🚀 PIPPIN ATR Projesi - Sunucuya Deploy"
echo ""

# Sunucu bilgileri
SERVER="root@139.59.163.105"
SERVER_DIR="/root/volensy_quik_gain/PIPPIN_ATR_2H"
LOCAL_DIR="/Users/eralpcukurova/volensy_quik_gain/volensy_quik_gain/PIPPIN_ATR_2H"
...
ssh "$SERVER" << 'ENDSSH'
cd /root/volensy_quik_gain/PIPPIN_ATR_2H
...
if [ -f "pippin_atr_live.service" ]; then
    cp pippin_atr_live.service /etc/systemd/system/
    systemctl daemon-reload
    echo "   ✅ Service dosyası kopyalandı"
    echo "   ✅ Systemd daemon reload edildi"
else
    echo "   ⚠️  pippin_atr_live.service bulunamadı!"
    exit 1
fi
...
systemctl enable pippin_atr_live.service
echo "   ✅ Service etkinleştirildi (boot'ta başlayacak)"

# Mevcut service'i durdur (varsa)
echo ""
echo "⏸️  Mevcut service durduruluyor (varsa)..."
systemctl stop pippin_atr_live.service 2>/dev/null || true
sleep 1

# Service başlat
echo ""
echo "▶️  Service başlatılıyor..."
systemctl start pippin_atr_live.service
sleep 3

# Service durumu kontrol
echo ""
echo "📊 Service durumu kontrol ediliyor..."
if systemctl is-active --quiet pippin_atr_live.service; then
    echo "   ✅ Service çalışıyor!"
    systemctl status pippin_atr_live.service --no-pager -l | head -20
else
    echo "   ⚠️  Service çalışmıyor. Logları kontrol edin:"
    echo "      tail -f $SERVER_DIR/runs/pippin_atr_live.log"
    echo "      journalctl -u pippin_atr_live.service -n 50 --no-pager"
    exit 1
fi
...
echo "   ssh $SERVER 'tail -f $SERVER_DIR/runs/pippin_atr_live.log'"
echo ""
echo "📋 Service durumu:"
echo "   ssh $SERVER 'systemctl status pippin_atr_live.service'"
echo ""
echo "🔄 Service yönetimi:"
echo "   ssh $SERVER 'systemctl restart pippin_atr_live.service'  # Yeniden başlat"
echo "   ssh $SERVER 'systemctl stop pippin_atr_live.service'      # Durdur"
echo "   ssh $SERVER 'systemctl start pippin_atr_live.service'      # Başlat"
