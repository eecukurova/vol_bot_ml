#!/bin/bash
# SOL Projesi - Sunucuya Deploy Scripti

set -e

echo "🚀 SOL Projesi - Sunucuya Deploy"
echo ""

# Sunucu bilgileri
SERVER="root@159.65.94.27"
SERVER_DIR="/root/ATR/SOL"
LOCAL_DIR="/Users/ahmet/ATR/SOL"
...
if [ ! -f "configs/train_3m.json" ]; then
    echo "   ❌ configs/train_3m.json bulunamadı!"
    exit 1
fi
...
systemctl stop sol_live.service 2>/dev/null || true
sleep 1
...
if systemctl is-active --quiet sol_live.service; then
    echo "   ✅ Service çalışıyor!"
    systemctl status sol_live.service --no-pager -l | head -20
else
    echo "   ⚠️  Service çalışmıyor. Logları kontrol edin:"
    echo "      tail -f /root/ATR/SOL/runs/sol_live.log"
    echo "      journalctl -u sol_live.service -n 50 --no-pager"
    exit 1
fi
...
echo "   ssh $SERVER 'tail -f $SERVER_DIR/runs/sol_live.log'"
echo ""
echo "📋 Service durumu:"
echo "   ssh $SERVER 'systemctl status sol_live.service'"
echo ""
echo "🔄 Service yönetimi:"
echo "   ssh $SERVER 'systemctl restart sol_live.service'  # Yeniden başlat"
echo "   ssh $SERVER 'systemctl stop sol_live.service'      # Durdur"
echo "   ssh $SERVER 'systemctl start sol_live.service'      # Başlat"
