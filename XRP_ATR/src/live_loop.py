    ...
    try:
        ...
        if not positions_found:
            try:
                ...
                # Filter for symbol positions (extract base symbol from symbol like XRPUSDT -> XRP)
...
def send_order(
    ...
    # Symbol is passed as parameter (default: XRPUSDT)
...
def send_telegram_alert(payload: Dict) -> None:
    ...
    try:
        ...
        symbol = payload.get("symbol", "XRPUSDT")
        ...
        message = f"""
📊 ATR XRP Signal{shadow_indicator}
📌 Pair: {symbol}
📊 Side: {side}
💰 Entry: ${entry:,.2f}
🎯 TP: ${tp:,.2f}
