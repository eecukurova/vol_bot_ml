"""
Backtesting engine for NASDAQ Strategy Optimizer.

This module implements a comprehensive backtesting engine for NASDAQ stocks with position management,
...
class Backtester:
    ...
    def __init__(self, initial_capital: float = 10000.0, fee_bps: float = 1.0, 
                 slippage_bps: float = 2.0, position_size_pct: float = 0.1, 
                 leverage: float = 1.0):
        """
        Initialize NASDAQ backtester with stock-specific parameters.
        
        Args:
            initial_capital: Initial capital amount
            fee_bps: Fee in basis points (0.01% = 1 bps for stocks)
            slippage_bps: Slippage in basis points (0.02% = 2 bps for stocks)
            position_size_pct: Position size as percentage of capital (0.1 = 10%)
            leverage: Leverage multiplier (1.0 = no leverage for stocks)
        """
        self.initial_capital = initial_capital
        self.fee_bps = fee_bps / 10000.0  # Convert to decimal
        self.slippage_bps = slippage_bps / 10000.0  # Convert to decimal
        
        # NASDAQ-specific risk management
        self.position_size_pct = position_size_pct  # 10% of capital per trade
        self.leverage = leverage  # No leverage for stocks
    ...
    def calculate_position_size(self, price: float, stop_loss: float) -> float:
        """
        Calculate position size based on percentage of capital for NASDAQ stocks.
        ...
        # Round to whole shares (stocks are traded in whole shares)
        return int(shares)
    ...
    def open_position(self, timestamp: datetime, price: float, side: str, 
        ...
        margin_required = position_value * self.position_size_pct  # Use position_size_pct
    ...
    def close_position(self, timestamp: datetime, price: float, reason: str) -> Optional[Trade]:
        ...
        margin_return = trade.quantity * trade.entry_price * self.position_size_pct  # Return margin
