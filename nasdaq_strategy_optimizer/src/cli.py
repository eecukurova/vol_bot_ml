"""
Command-line interface for NASDAQ Strategy Optimizer.

This module provides a comprehensive CLI for NASDAQ stock optimization,
walk-forward analysis, and reporting using Yahoo Finance data.
"""
...
from .config import get_config, validate_symbols, validate_timeframes
...
app = typer.Typer(
    name="nasdaq-strategy-optimizer",
    help="NASDAQ Strategy Optimizer - ATR SuperTrend for NASDAQ Stocks",
...
# NASDAQ-specific commands
@app.command()
def optimize_nasdaq(
    symbols: str = typer.Option("AAPL,AMD,NVDA,TSLA", help="NASDAQ symbols to optimize (comma-separated)"),
    timeframes: str = typer.Option("1d,1wk", help="Timeframes to test (comma-separated)"),
    strategy: str = typer.Option("atr_supertrend", help="Strategy type"),
    params_file: Optional[str] = typer.Option(None, help="JSON file with parameter ranges"),
    output_dir: str = typer.Option("./reports/nasdaq", help="Output directory for results"),
    jobs: int = typer.Option(4, help="Number of parallel jobs"),
    top_n: int = typer.Option(10, help="Number of top results to keep")
):
    """Optimize NASDAQ stocks with ATR SuperTrend strategy."""
    
    logger.info("🚀 Starting NASDAQ optimization")
    ...
    # Validate inputs
    valid_symbols = validate_symbols(symbol_list)
    valid_timeframes = validate_timeframes(timeframe_list)
    
    if not valid_symbols:
        logger.error("❌ No valid NASDAQ symbols provided")
        raise typer.Exit(1)
    
    logger.info(f"📊 Symbols: {valid_symbols}")
    ...
    if params_file:
        ...
        # Use default NASDAQ parameters
        config = get_config()
        param_space = config.strategy.param_space
    
    logger.info(f"🔧 Parameter space: {len(param_space)} combinations")
    ...
    try:
        ...
        results_file = output_path / f"nasdaq_optimization_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        ...
        for i, result in enumerate(results['top_results'][:5], 1):
            print(f"{i}. {result['symbol']} {result['timeframe']} - Return: {result['total_return']:.2%}, Sharpe: {result['sharpe_ratio']:.2f}")
...
@app.command()
def test_strategy(
    symbol: str = typer.Option("AAPL", help="NASDAQ symbol to test"),
    timeframe: str = typer.Option("1d", help="Timeframe to test"),
    strategy: str = typer.Option("atr_supertrend", help="Strategy type"),
    params_string: str = typer.Option("a=3 c=10 st_factor=1.5", help="Parameter string"),
    days: int = typer.Option(180, help="Number of days of data to use")
):
    """Test strategy on a single NASDAQ symbol."""
    
    logger.info(f"🧪 Testing {strategy} on {symbol} {timeframe}")
    
    # Parse parameters
    params = parse_param_string(params_string)[0]
    ...
    data = loader.get_ohlcv(symbol, timeframe, start_date, end_date)
    ...
    # Create strategy
    strategy_instance = create_nasdaq_strategy(strategy, params)
    ...
    try:
        ...
        # Print results
        print(f"\n📈 STRATEGY TEST RESULTS - {symbol} {timeframe}")
        print(f"{'='*50}")
        ...
        if result.signals:
            ...
            for signal in result.signals[-5:]:  # Show last 5 signals
                print(f"  {signal.timestamp.strftime('%Y-%m-%d')} - {signal.signal_type} @ ${signal.price:.2f} (conf: {signal.confidence:.2f})")
...
def list_symbols(
    ...
    min_volume: Optional[int] = typer.Option(None, help="Minimum daily volume"),
    limit: int = typer.Option(20, help="Maximum number of symbols to show")
):
    """List available NASDAQ symbols."""
    ...
    if sector:
        symbols = nasdaq_provider.get_symbols_by_sector(sector)
        print(f"\n📊 {sector} Sector Symbols:")
    elif min_volume:
        symbols = nasdaq_provider.get_high_volume_symbols(min_volume)
        print(f"\n📊 High-Volume Symbols (min: {min_volume:,}):")
    else:
        symbols = nasdaq_provider.get_available_symbols()
        print(f"\n📊 All Available NASDAQ Symbols:")
    
    # Show symbols with info
    for i, symbol in enumerate(symbols[:limit], 1):
        info = nasdaq_provider.get_symbol_info(symbol)
        if info:
            print(f"{i:2d}. {symbol:5s} - {info.name:30s} ({info.sector:20s}) - Vol: {info.volume_avg:,}")
        else:
            print(f"{i:2d}. {symbol:5s}")
