#!/usr/bin/env python3
"""
Optimized Multiple Strategy Testing - Bollinger Bands, EMA, RSI, MACD
"""
...
def test_optimized_strategies():
    """Test optimized strategies with different parameter sets"""
    print('🚀 OPTİMİZE EDİLMİŞ ÇOKLU STRATEJİ TESTİ')
    print('='*70)
    ...
    strategies = [
        {
            'name': 'Bollinger Bands (Optimize)',
            'class': BollingerBandsStrategy,
            'params': {'period': 15, 'std_dev': 1.5, 'stop_loss': 0.015, 'take_profit': 0.03}
        },
        {
            'name': 'EMA Crossover (Fast)',
            'class': EMAStrategy,
            'params': {'fast_ema': 8, 'slow_ema': 21, 'stop_loss': 0.015, 'take_profit': 0.03}
        },
        {
            'name': 'RSI (Agresif)',
            'class': RSIStrategy,
            'params': {'period': 10, 'oversold': 25, 'overbought': 75, 'stop_loss': 0.015, 'take_profit': 0.03}
        },
        {
            'name': 'Bollinger Bands (Ultra Agresif)',
            'class': BollingerBandsStrategy,
            'params': {'period': 10, 'std_dev': 1.2, 'stop_loss': 0.01, 'take_profit': 0.02}
        },
        {
            'name': 'EMA Crossover (Ultra Fast)',
            'class': EMAStrategy,
            'params': {'fast_ema': 5, 'slow_ema': 13, 'stop_loss': 0.01, 'take_profit': 0.02}
        },
        {
            'name': 'RSI (Ultra Agresif)',
            'class': RSIStrategy,
            'params': {'period': 7, 'oversold': 20, 'overbought': 80, 'stop_loss': 0.01, 'take_profit': 0.02}
        }
    ]
    ...
    if all_results:
        print(f'\n🏆 EN İYİ OPTİMİZE EDİLMİŞ STRATEJİLER')
        print('='*70)
        ...
        # Compare with SUPERTREND
        print(f'\n🆚 SUPERTREND İLE KARŞILAŞTIRMA:')
        print(f'   SUPERTREND (Optimize): +26.99%')
        best_strategy_return = sorted_strategies[0][1]['avg_return']
        if best_strategy_return > 0.2699:
            print(f'   🎉 YENİ REKOR! {sorted_strategies[0][0]}: {best_strategy_return:.2%}')
        else:
            print(f'   📊 En İyi Yeni Strateji: {sorted_strategies[0][0]}: {best_strategy_return:.2%}')
