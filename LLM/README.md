# Volensy LLM Futures – 0.5% TP Scalper (Transformer)

A Transformer-based sequence classifier for BTCUSDT futures scalping with 0.5% take-profit targets and adaptive stop-loss via grid search.

## 🎯 Approach

- **Target**: 0.5% TP scalping on BTCUSDT
- **Model**: Small Transformer encoder (64-dim, 2 layers, 4 heads)
- **Labeling**: Triple-barrier first-touch with configurable TP/SL
- **Validation**: Time-based split, weighted CE loss for class imbalance
- **Optimization**: Grid search on SL thresholds and decision thresholds
- **Backtest**: Commission (0.05%) + slippage included

## 📦 Installation

```bash
make venv          # Create virtual environment
source venv/bin/activate
make install       # Install dependencies
make fmt           # Format code
make lint          # Lint code
```

## 🔄 Workflow

### 1. Data Preparation

Download 3-minute klines from Binance:

```bash
python scripts/download_binance_klines.py --symbol BTCUSDT --interval 3m --start 2024-01-01
```

Data saved to `data/BTCUSDT_3m.csv` with columns: `time,open,high,low,close,volume`.

### 2. Training

```bash
make train
# or
python scripts/train_runner.py --symbol BTCUSDT --timeframe 3m
```

Trainer will:
- Load CSV data
- Add features (EMA distances, RSI, volume spikes, z-scores)
- Apply triple-barrier labeling (TP=0.5%, SL candidates)
- Create windows (length=128)
- Train Transformer with validation split
- Save `models/seqcls.pt` and `models/feat_cols.json`

Configuration: `configs/train_3m.json`

### 3. Grid Search

Find optimal SL and decision thresholds:

```bash
make grid
# or
python scripts/gridsearch_runner.py --symbol BTCUSDT --timeframe 3m
```

Tests combinations of:
- `sl_pct`: [0.006, 0.008, 0.010]
- `thr_long/thr_short`: [0.55, 0.60, 0.65]

Outputs top-5 combinations with metrics (hit-rate, PF, trades, drawdown).

### 4. Backtest

Run backtest with trained model and best thresholds:

```bash
make backtest
# or
python scripts/backtest_runner.py --symbol BTCUSDT --timeframe 3m \
    --sl-pct 0.008 --thr-long 0.60 --thr-short 0.60
```

Outputs:
- Final equity curve
- Profit Factor
- Win-rate (%)
- Total trades
- Max drawdown

### 5. Live Loop Demo

Simulate live inference (no actual orders):

```bash
make live
# or
python scripts/live_demo_runner.py --symbol BTCUSDT --timeframe 3m \
    --sl-pct 0.008 --thr-long 0.60
```

Prints:
- Each bar decision
- ML probabilities
- TP/SL values
- Telegram-style alert payloads

## 🚀 Production Live Runner

The production process runs via `run_llm_live.py` and a systemd service. It loads the trained model, fetches new 3m bars, makes decisions, sends Futures orders with TP/SL, and posts Telegram alerts.

- Entry point: `run_llm_live.py`
- Service: `llm_live.service` (systemd)
- Working directory: `/root/ATR/LLM`
- Logs: `runs/llm_live.log`

### What it does
- Loads `models/seqcls.pt` and `models/feat_cols.json`.
- Reads config from `configs/llm_config.json` (API keys, leverage, trade amount, Telegram).
- Reads training/live settings from `configs/train_3m.json` (e.g. `timeframe`, `window`, `tp_pct`).
- Pulls latest `timeframe` klines from Binance Spot for features.
- Applies features and the Transformer to generate probabilities and a side (LONG/SHORT/FLAT).
- Applies regime filter (e.g. `ema50 > ema200`, `vol_spike > 0.8`).
- If eligible, places a market entry and attaches TP/SL (closePosition) on Binance Futures via `src.live_loop -> init_order_client -> src.order_client`.
- Sends a Telegram alert if enabled.

### Configuration (`configs/llm_config.json`)
Key fields used by the live runner:

```json
{
  "api_key": "...",
  "secret": "...",
  "symbol": "BTCUSDT",
  "trade_amount_usd": 100,
  "leverage": 5,
  "idempotency": { "state_file": "runs/llm_state.json", "retry_attempts": 3, "retry_delay": 1.0 },
  "sl_tp": { "trigger_source": "MARK_PRICE", "hedge_mode": false },
  "telegram": { "enabled": true, "bot_token": "...", "chat_id": "..." },
  "logging": { "level": "INFO", "file": "llm_trading.log" }
}
```

Update trade size or leverage by editing `trade_amount_usd` and `leverage`. The runner reads these dynamically on start.

### Service management

```bash
# Status
systemctl status llm_live.service --no-pager -l

# Start/Stop/Restart
systemctl start llm_live.service
systemctl stop llm_live.service
systemctl restart llm_live.service

# Enable on boot
systemctl enable llm_live.service
```

### Logs

```bash
tail -f runs/llm_live.log
```

Typical startup lines:

```
INFO:__main__:🚀 Starting LLM Live Signal Generator
INFO:src.live_loop:✅ Order client initialized
INFO:__main__:Model loaded: 17 features
INFO:__main__:📊 Monitoring: BTC/USDT 3m
```

### Time and timestamps
- Exchange data and orders are UTC-based; the server logs local time (+03). A UTC vs local mismatch may make entries look like “previous day.” This is expected.

## ⚙️ Runtime Behavior (Important)

- Bar cadence: runs when a new bar appears for the configured `timeframe` (default 3m). Sleeps between checks.
- Position sizing: `trade_amount_usd` notional with `leverage` → entry size computed from latest price.
- Orders: Market entry; TP/SL sent as `TAKE_PROFIT_MARKET`/`STOP_MARKET` with `closePosition=true`, `workingType=MARK_PRICE`.
- Safety: TP/SL stop prices are adjusted against mark price to avoid Binance `-2021 Order would immediately trigger` errors.
- State: Idempotent order client persists to `runs/llm_state.json` and dedupes/retries.

## 🧰 Troubleshooting

- No logs: check service status and permissions in `/root/ATR/LLM`.
- `-2021` errors: TP/SL safety adjustment is enabled; if seen, verify `workingType=MARK_PRICE` and price precision.
- No signals: thresholds too strict or regime filter rejects; adjust `thr_long/thr_short`, `sl_pct`, or regime.
- API errors: validate `api_key/secret`; ensure Futures trading is enabled on the account.

## 📁 Project Structure

```
LLM/
├── configs/
│   └── train_3m.json
├── data/              # CSV raw data
├── models/            # Saved weights + feat_cols.json
├── notebooks/         # Optional analysis
├── scripts/
│   ├── download_binance_klines.py
│   ├── train_runner.py
│   ├── backtest_runner.py
│   ├── gridsearch_runner.py
│   └── live_demo_runner.py
├── src/
│   ├── fetch_binance.py
│   ├── features.py
│   ├── labeling.py
│   ├── dataset.py
│   ├── models/
│   │   └── transformer.py
│   ├── train.py
│   ├── infer.py
│   ├── backtest_core.py
│   ├── gridsearch.py
│   ├── live_loop.py
│   └── utils.py
└── tests/
    ├── test_labeling.py
    ├── test_features.py
    └── test_model_forward.py
```

## 🧪 Testing

```bash
make test
```

Runs unit tests for labeling logic, feature engineering, and model forward pass.

## ⚠️ Warnings

- **Funding rates**: Not modeled in backtest (BTCUSDT perp funding ~0.01% per 8h)
- **Slippage**: Default 0; add realistic slippage for live trading
- **Latency**: 100ms+ execution can miss TP
- **Overfitting**: Use walk-forward validation for production
- **Regime dependency**: Model trained on 2024 data may fail in 2025 if regime shifts
- **Risk**: Start with paper trading; 0.5% TP requires high hit-rate to be profitable

## 🔄 Automated Retraining

The project includes automated retraining with walk-forward validation and intelligent model selection:

### Features

1. **Haftalık Retraining**: Otomatik olarak son 7 günün verilerini indirir ve modeli yeniden eğitir
2. **Walk-Forward Validation**: Model performansını son 2 haftalık görülmemiş veriler üzerinde test eder
3. **Otomatik Model Seçimi**: Yeni model mevcut modelden en az %5 daha iyiyse otomatik olarak deploy eder
4. **Güvenli Deployment**: Eski modeli otomatik yedekler (`models/backups/`)

### Retraining Script

```bash
# Manuel retraining
cd LLM
python scripts/retrain_runner.py --config configs/train_3m.json

# Parametreler
--test-weeks 2          # Walk-forward için kaç hafta veri kullanılacak (default: 2)
--min-improvement 0.05  # Minimum iyileştirme oranı (default: 0.05 = %5)
--days-back 7           # Kaç günlük yeni veri indirilecek (default: 7)
--force                 # Zorla deploy et (karşılaştırma yapmadan)
```

### Cron Job Kurulumu (Haftalık Retraining)

```bash
# Crontab düzenle
crontab -e

# Her Pazar gecesi 02:00'de çalıştır (venv kullanarak)
0 2 * * 0 cd /root/ATR/LLM && /root/ATR/LLM/venv/bin/python scripts/retrain_runner.py --config configs/train_3m.json --test-weeks 2 --min-improvement 0.05 >> runs/retrain.log 2>&1
```

### Model Karşılaştırma Metrikleri

Retraining script, model performansını şu metriklerle karşılaştırır:

- **Profit Factor** (40% ağırlık): En önemli metrik
- **Win Rate** (30% ağırlık): Kazanan işlem oranı
- **Max Drawdown** (20% ağırlık): Maksimum düşüş
- **Final Equity** (10% ağırlık): Final sermaye

Yeni model, ağırlıklı skor bakımından mevcut modelden en az %5 daha iyi olmalıdır.

### Çıktı Örneği

```
🔄 AUTOMATED RETRAINING
============================================================

[1/5] 📥 Downloading latest data...
📥 Downloading latest data: 2025-10-25 to 2025-11-01
...

[3/5] 🧪 Testing current model (walk-forward validation)...
   Old Model Performance:
      Profit Factor: 1.15
      Win Rate: 52.3%
      Max Drawdown: 8.2%

[4/5] 🎓 Training new model...
   Train: 12543, Val: 3135

[5/5] 🧪 Testing new model (walk-forward validation)...
   New Model Performance:
      Profit Factor: 1.28
      Win Rate: 55.1%
      Max Drawdown: 6.8%

[6/6] 🔍 Comparing models...
📊 Model Comparison:
   Old Score: 0.6234
   New Score: 0.6912
   Improvement: 10.87%
✅ New model is better (≥5% improvement)

🚀 Deploying new model...
💾 Backed up model to models/backups/seqcls_20251101_020000.pt

✅ RETRAINING COMPLETE - New model deployed!
```

## 🎯 Integration Hooks

Orders and alerts are already integrated:

- Orders: `src/live_loop.init_order_client` wires to `src.order_client` (Binance Futures via ccxt) using idempotent client and `runs/llm_state.json`.
- Alerts: `src.live_loop.send_telegram_alert` uses `telegram` block from `llm_config.json` when enabled.
