...
def run_backtest(
    ...
    Args:
        ...
        include_funding: Whether to include funding costs in PnL
    ...
    for i in range(window, n):
        ...
        # Apply funding costs (if enabled)
        if include_funding:
            # Funding is paid every 8 hours
            # Estimate funding periods during the trade
            bars_held = j - (i + 1)  # Number of bars the position was held
            # Assuming 3-minute bars: 8 hours = 160 bars
            funding_periods = bars_held / 160.0
            
            # Funding cost: if LONG, pay funding (positive rate = cost)
            #                if SHORT, receive funding (positive rate = gain)
            funding_cost = funding_rate * funding_periods
            if side == "LONG":
                pnl -= funding_cost  # Long pays funding
            else:  # SHORT
                pnl += funding_cost  # Short receives funding (if rate positive)
