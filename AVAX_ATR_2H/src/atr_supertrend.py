...
def get_atr_supertrend_signals(
    ...
    xATRTrailingStop, pos, src = calculate_atr_trailing_stop(
        df, atr_period=atr_period, key_value=key_value, use_heikin_ashi=use_heikin_ashi
    )
    ...
    # Determine side - Pine Script'te SADECE buy/sell sinyalleri kullanılır
    # Super Trend sadece alertcondition için, sinyal değil
    side = None
    if buy_signal:
        side = "LONG"
    elif sell_signal:
    ...
    signal_info = {
        ...
        "buy_super_trend": bool(buy_super_trend),
        "sell_super_trend": bool(sell_super_trend),
        "above": bool(above),
        "below": bool(below)
    }
