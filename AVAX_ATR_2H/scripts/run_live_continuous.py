#!/usr/bin/env python3
"""AVAX ATR Live Signal Generator - ATR + Super Trend Strategy."""
import time
import ccxt
import pandas as pd
import json
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("runs/avax_atr_live.log"),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger(__name__)

def fetch_latest_bars(symbol="AVAXUSDT", timeframe="2h", limit=200):
...
def fetch_trend_bars(symbol="AVAXUSDT", timeframe="6h", limit=200):
...
def main():
    logger.info("🚀 Starting AVAX ATR Live Signal Generator (ATR + Super Trend Strategy)")
    ...
    symbol = llm_cfg.get("symbol", "AVAXUSDT")
