#!/bin/bash
# AVAX ATR Projesi - Sunucuya Deploy Scripti

set -e

echo "🚀 AVAX ATR Projesi - Sunucuya Deploy"
echo ""

# Sunucu bilgileri
SERVER="root@159.65.94.27"
SERVER_DIR="/opt/avax_atr_2h"
LOCAL_DIR="/Users/ahmet/ATR/AVAX_ATR_2H"
...
if [ -f "avax_atr_live.service" ]; then
    cp avax_atr_live.service /etc/systemd/system/
    systemctl daemon-reload
    echo "   ✅ Service dosyası kopyalandı"
    echo "   ✅ Systemd daemon reload edildi"
else
    echo "   ⚠️  avax_atr_live.service bulunamadı!"
    exit 1
fi
...
systemctl stop avax_atr_live.service 2>/dev/null || true
sleep 1
...
if systemctl is-active --quiet avax_atr_live.service; then
    echo "   ✅ Service çalışıyor!"
    systemctl status avax_atr_live.service --no-pager -l | head -20
else
    echo "   ⚠️  Service çalışmıyor. Logları kontrol edin:"
    echo "      tail -f /opt/avax_atr_2h/runs/avax_atr_live.log"
    echo "      journalctl -u avax_atr_live.service -n 50 --no-pager"
    exit 1
fi
...
echo "   ssh $SERVER 'tail -f $SERVER_DIR/runs/avax_atr_live.log'"
echo ""
echo "📋 Service durumu:"
echo "   ssh $SERVER 'systemctl status avax_atr_live.service'"
echo ""
echo "🔄 Service yönetimi:"
echo "   ssh $SERVER 'systemctl restart avax_atr_live.service'  # Yeniden başlat"
echo "   ssh $SERVER 'systemctl stop avax_atr_live.service'      # Durdur"
echo "   ssh $SERVER 'systemctl start avax_atr_live.service'      # Başlat"
