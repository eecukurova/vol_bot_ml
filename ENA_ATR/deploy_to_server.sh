#!/bin/bash
# ENA ATR Projesi - Sunucuya Deploy Scripti

set -e

echo "🚀 ENA ATR Projesi - Sunucuya Deploy"
echo ""

# Sunucu bilgileri
SERVER="root@159.65.94.27"
SERVER_DIR="/root/ATR/ENA_ATR"
LOCAL_DIR="/Users/ahmet/ATR/ENA_ATR"
...
if [ -f "ena_atr_live.service" ]; then
    cp ena_atr_live.service /etc/systemd/system/
    systemctl daemon-reload
    echo "   ✅ Service dosyası kopyalandı"
    echo "   ✅ Systemd daemon reload edildi"
else
    echo "   ⚠️  ena_atr_live.service bulunamadı!"
    exit 1
fi
...
systemctl enable ena_atr_live.service
echo "   ✅ Service etkinleştirildi (boot'ta başlayacak)"

# Mevcut service'i durdur (varsa)
echo ""
echo "⏸️  Mevcut service durduruluyor (varsa)..."
systemctl stop ena_atr_live.service 2>/dev/null || true
sleep 1

# Service başlat
echo ""
echo "▶️  Service başlatılıyor..."
systemctl start ena_atr_live.service
sleep 3

# Service durumu kontrol
echo ""
echo "📊 Service durumu kontrol ediliyor..."
if systemctl is-active --quiet ena_atr_live.service; then
    echo "   ✅ Service çalışıyor!"
    systemctl status ena_atr_live.service --no-pager -l | head -20
else
    echo "   ⚠️  Service çalışmıyor. Logları kontrol edin:"
    echo "      tail -f /root/ATR/ENA_ATR/runs/ena_atr_live.log"
    echo "      journalctl -u ena_atr_live.service -n 50 --no-pager"
    exit 1
fi
...
echo "   ssh $SERVER 'tail -f $SERVER_DIR/runs/ena_atr_live.log'"
echo ""
echo "📋 Service durumu:"
echo "   ssh $SERVER 'systemctl status ena_atr_live.service'"
echo ""
echo "🔄 Service yönetimi:"
echo "   ssh $SERVER 'systemctl restart ena_atr_live.service'  # Yeniden başlat"
echo "   ssh $SERVER 'systemctl stop ena_atr_live.service'      # Durdur"
echo "   ssh $SERVER 'systemctl start ena_atr_live.service'      # Başlat"
