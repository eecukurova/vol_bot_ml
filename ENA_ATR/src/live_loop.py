    ...
    try:
        ...
        if not positions_found:
            try:
                ...
                # Filter for symbol positions (extract base symbol from symbol like ENAUSDT -> ENA)
...
def send_order(
    ...
    # Symbol is passed as parameter (default: ENAUSDT)
...
def send_telegram_alert(payload: Dict) -> None:
    ...
    try:
        ...
        symbol = payload.get("symbol", "ENAUSDT")
        ...
        message = f"""
📊 ATR ENA Signal{shadow_indicator}
📌 Pair: {symbol}
📊 Side: {side}
💰 Entry: ${entry:,.2f}
🎯 TP: ${tp:,.2f}
