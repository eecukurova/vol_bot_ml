# ENA ATR Live Trading – ATR + Super Trend Strategy

ENA ATR Live Trading Signal Generator using ATR + Super Trend strategy for ENAUSDT futures trading with configurable take-profit and stop-loss.

## 🎯 Approach

- **Target**: ENAUSDT futures trading with ATR + Super Trend signals
...
```bash
python scripts/download_binance_klines.py --symbol ENAUSDT --interval 15m --start 2024-01-01
```

Data saved to `data/ENAUSDT_15m.csv` with columns: `time,open,high,low,close,volume`.
...
make train
# or
python scripts/train_runner.py --symbol ENAUSDT --timeframe 15m
```
...
make grid
# or
python scripts/gridsearch_runner.py --symbol ENAUSDT --timeframe 15m
```
...
make backtest
# or
python scripts/backtest_runner.py --symbol ENAUSDT --timeframe 15m \
...
make live
# or
python scripts/live_demo_runner.py --symbol ENAUSDT --timeframe 15m \
...
- Working directory: `/root/ATR/ENA_ATR`
- Logs: `runs/ena_atr_live.log`
...
INFO:__main__:📊 Monitoring: ENAUSDT 15m
```
...
- No logs: check service status and permissions in `/root/ATR/ENA_ATR`.
...
- **Funding rates**: Not modeled in backtest (ENAUSDT perp funding ~0.01% per 8h)
