#!/usr/bin/env python3
"""
RSI Ultra Agresif Strategy - Comprehensive NASDAQ Stock Testing
Based on Pine Script: rsi_ultra_agresif_strategy.pine
"""
...
class RSIUltraAgresifStrategy:
    def __init__(self, params: Dict[str, Any]):
        ...
        self.stop_loss = params.get('stop_loss', 0.01)  # 1.0%
        self.take_profit = params.get('take_profit', 0.02)  # 2.0%
    ...
    def generate_signals(self, df: pd.DataFrame) -> List[Dict[str, Any]]:
        ...
        for i in range(1, len(df)):
            ...
            buy_signal = (previous['rsi'] <= self.rsi_oversold and 
                         current['rsi'] > self.rsi_oversold)
            
            # Sell Signal: RSI crosses below overbought level
            sell_signal = (previous['rsi'] >= self.rsi_overbought and 
...
def test_rsi_ultra_agresif_nasdaq_stocks():
    """Test RSI Ultra Agresif Strategy on comprehensive NASDAQ stocks"""
    print('🚀 RSI ULTRA AGRESİF STRATEJİSİ - KAPSAMLI NASDAQ HİSSE TESTİ')
    print('='*80)
    
    # Comprehensive NASDAQ stock list (same as previous tests)
    nasdaq_stocks = [
        # Tech Giants
    ...
    print(f'🎯 Strateji: RSI Ultra Agresif (Pine Script Parametreleri)')
    print(f'📈 Parametreler: RSI=7, Oversold=20, Overbought=80, SL=1%, TP=2%')
    print()
    
    # Strategy parameters (from Pine Script - ultra agresif)
    strategy_params = {
        'rsi_period': 7,
        'rsi_oversold': 20,
        'rsi_overbought': 80,
    ...
    for i, symbol in enumerate(unique_stocks, 1):
        try:
            ...
            # Create strategy
            strategy = RSIUltraAgresifStrategy(strategy_params)
    ...
    if results:
        ...
        print(f'\n🏆 RSI ULTRA AGRESİF - EN İYİ PERFORMANS GÖSTEREN HİSSELER')
        print('='*80)
        ...
        print(f'\n📊 RSI ULTRA AGRESİF GENEL İSTATİSTİKLER:')
...
if __name__ == "__main__":
    test_rsi_ultra_agresif_nasdaq_stocks()
